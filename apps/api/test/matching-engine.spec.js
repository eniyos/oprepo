...
const matching_engine_service_1 = require("../src/modules/recommendation/matching-engine.service");
describe('MatchingEngine', () => {
    let engine;
    beforeEach(() => {
        engine = new matching_engine_service_1.MatchingEngine();
    });
    ...
    describe('scoreHybrid', () => {
        it('should blend ML scores with rule-based scores', async () => {
            ...
            const results = await engine.scoreHybrid(developer, candidates, 'repos', async () => [0.9, 0.2]);
            expect(results.length).toBe(2);
            expect(results[0].score).toBeGreaterThan(results[1].score);
        ...
        it('should add a ML similarity reason when mlScore is high', async () => {
            ...
            const results = await engine.scoreHybrid(developer, candidates, 'repos', async () => [0.95]);
        ...
        it('should fall through with empty ML similarity gracefully', async () => {
            ...
            const results = await engine.scoreHybrid(developer, candidates, 'repos', async () => []);
            expect(results.length).toBe(1);
            expect(results[0].score).toBeGreaterThanOrEqual(0);
        });
        it('should fall through when similarityFn throws', async () => {
            ...
            const results = await engine.scoreHybrid(developer, candidates, 'repos', async () => { throw new Error('ML down'); });
            expect(results.length).toBe(1);
            expect(results[0].score).toBeGreaterThanOrEqual(0);
        });
    });
});
//# sourceMappingURL=matching-engine.spec.js.map